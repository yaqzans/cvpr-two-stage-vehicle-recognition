"""
Stage 2 v2 - ConvNeXt-Tiny training for BangladeshVehicleClassification

Same dataset, same model, same optimizer, same batch size and learning rate as
train_stage2_convnext_tiny_fixed.py (v1). Three deliberate changes only, so the
comparison against v1 measures those three things and nothing else.

------------------------------------------------------------------------------
CHANGE 1 - Preprocessing keeps the whole object
------------------------------------------------------------------------------
v1 evaluated with:

    Resize(256)          # scales the SHORTER side to 256, keeps aspect ratio
    CenterCrop(224)      # then takes a 224x224 square out of the middle

On a tight object crop that discards most of the object. Measured over the
real crop aspect ratios in this dataset, the fraction of the long dimension
that survives is:

    person          32%      rickshaw        57%      bicycle     58%
    motorcycle      61%      truck           72%      bus         68%

A person crop is roughly 60x195 px. Resize makes it 256x832. CenterCrop then
keeps a 224x224 window from the middle - torso only, no head, no legs. A truck
loses its front and rear, which is exactly what separates it from a covered
van. v1's two worst classes were truck (F1 0.478) and covered_van (F1 0.681).

v2 pads the shorter side to make the crop square, then resizes to 224. Aspect
ratio is preserved and 100% of the object survives.

------------------------------------------------------------------------------
CHANGE 2 - The cosine schedule actually runs
------------------------------------------------------------------------------
v1 set CosineAnnealingLR(T_max=100) but early-stopped at epoch 28, so the
learning rate only fell from 1.00e-4 to 8.3e-5. The model never reached the
low-LR consolidation phase where fine-grained classes are usually won.

v1's best epoch was 8 of 28: this model converges fast. So v2 sets a realistic
budget (30 epochs), matches T_max to it, adds a short warmup, and deliberately
disables early stopping so the schedule always completes. Best-checkpoint
selection on validation macro F1 is unchanged, so nothing is lost by running
to the end.

------------------------------------------------------------------------------
CHANGE 3 - Training boxes are jittered to match Stage 1
------------------------------------------------------------------------------
The crops on disk were cut along human-drawn boxes. At runtime Stage 2 is fed
boxes from YOLO, which sit at ~0.89 mean IoU with the truth - slightly tight,
slightly loose, slightly shifted. v1 trained on perfect boxes and is deployed
on jittered ones. v2 jitters each edge during training so the two match.

------------------------------------------------------------------------------
Deliberately NOT changed
------------------------------------------------------------------------------
Model, batch size, learning rate, weight decay, label smoothing, class
weights, optimizer, seed. Extra regularisation (stochastic depth, random
erasing) is wired up but defaults to OFF - see EXTRAS below - so this run
isolates the three changes above.

Outputs go to runs/stage2_convnext_tiny_v2/ so v1's results are preserved for
comparison.

Run from the project root:
    python scripts/train_stage2_convnext_tiny_v2.py
"""
...
RUN_DIR = PROJECT_ROOT / "runs" / "stage2_convnext_tiny_v2"
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("STAGE 2 v2 - CONVNEXT-TINY")
    print("=" * 70)
    ...
    print()
    print("Changes vs v1:")
    print("  1. PadToSquare + Resize instead of Resize+CenterCrop")
    print(f"  2. Cosine T_max={EPOCHS} with {WARMUP_EPOCHS}-epoch warmup, early stopping off")
    print(f"  3. RandomBoxJitter(max_frac={BOX_JITTER_FRAC}, p={BOX_JITTER_PROB})")
    print()
    ...
    with open(CONFIG_PATH, "w", encoding="utf-8") as f:
        json.dump({
            "version": "v2",
            "changes_vs_v1": [
                "PadToSquare + Resize replaces Resize(256) + CenterCrop(224)",
                f"cosine T_max={EPOCHS} with {WARMUP_EPOCHS}-epoch warmup; early stopping disabled",
                f"RandomBoxJitter(max_frac={BOX_JITTER_FRAC}, p={BOX_JITTER_PROB})",
            ],
            "model": MODEL_NAME,
    ...
    print()
    print("v1 reference: accuracy 0.9466, macro F1 0.8691, "
          "truck F1 0.4783, covered_van F1 0.6809")
    print()
